				...
				if (callsite.taggedWith(XCSG.StaticDispatchCallSite)) {
					...
					for (GraphElement Targetmethod : methods) {
						...
						if (name.contains(Targetname) && !(Targetmethod.taggedWith(XCSG.Constructor)
							...
							Q Calledges = Common.universe().edgesTaggedWithAll("RA");
...
//							.nodesTaggedWithAll(XCSG.Method);
	...
	public static void WriteInFile() throws IOException {
		...
		Q cha = Common.universe().edgesTaggedWithAll("RA").retainEdges();
		AtlasSet<GraphElement> callsitecha = Common.universe().nodesTaggedWithAny("RA").retainEdges().eval().nodes();
		...
		pw.write(Common.universe().edgesTaggedWithAll("RA").retainEdges().eval().nodes().size() + ","
				+ Common.universe().edgesTaggedWithAll("RA").retainEdges().eval().edges().size() + ","
				+ callsites.size() + ","
